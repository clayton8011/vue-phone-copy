<template>
    <div class="shop_container">
                                ...
                                <ul>
                                    <li v-for="(item,index) in cartFoodList" :key="index" class="cart_food_li">
                                        ...
                                        <section class="cart_list_control">
                                            ...
                                            <svg class="cart_add" @click="addToCart(item.categgory_id,item.item_id,item.food_id,item.name,item.price,item.specs)">
                    ...
                    <!--<section  type="2">-->
                        <section>
                            ...
                            <ul class="tag_list_ul">
                                <li v-for="(item ,index) in ratingTagsList" :key="index" :class="{unsatisfied:item.unstatisfied,tagActivity:ratingTageIndex === index}" @click="changeTgeIndex(index,item.name)">{{item.name}}({{item.count}})</li>
                            </ul>
                                    ...
                                    <section class="rating_list_details">
                                        ...
                                        <ul class="food_img_ul">
                                            <li v-for="(item,index) in item.item_ratings" :key="index">
                                                <img :src="getImgPath(itiem.image_hash)" v-if="item.image_path">
                                            </li>
                                        </ul>
        ...
        <transition name="router_slid" mode="out-in">
...
<script>
    ...
    export default {
       ...
       components:{
        ...
        methods:{
            ...mapMutations(['RECORD_ADDRESS','ADD_CART','REDUCE_CART','INIT_BUYCART','CLEAR_CART','RECORD_SHOPDETAIL']),
            async initData(){
                if(!this.latitude){
            ...
            listenScroll(ele){
                ...
                const wrapperMenu=new BScroll('#wrapper_menu',{click:true});
                    ...
                    if (this.shopCart&&this.shopCart[item.foods[0].category_id]) {
                        ...
                        Object.keys(this.shopCart[item.foods[0].category_id]).forEach(itemid => {
                            Object.keys(this.shopCart[item.foods[0].category_id][itemid]).forEach(foodid => {
                                ...
                                num += foodItem.num;
                                if (item.type == 1) {
                                    this.totalPrice += foodItem.num*foodItem.price; b
                                    if (foodItem.num > 0) {
